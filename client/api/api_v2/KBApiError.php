...
class KBApiError
{
    ...
    static $codes = array(
        ...
        5 => array('You are not allowed access this resource', 401), // no access, no priv 
        ...
        26 => array('Missing or invalid data', 400), // form validation
    ...
    // for form validation 
    static $error_messages_data = array(
        'required' => 'Missing fields(s): %s',
        'invalid'  => 'Invalid fields(s): %s'
    );
    ...
    // will be executed 
    static function shutdownCheckPriv() {
        self::error(5);
    }    
    ...
    static function parseObjErrorMsg($errors) {
        // foreach($errors['key'] as $k => $v) {
        //     // foreach($v as $v2) {
        //         $msg = self::$error_messages_data[$v['rule']];
        //     // }
        // }
        
        $error = $errors['key'][0];
        $msg = self::$error_messages_data[$error['rule']];
        $msg = sprintf($msg, implode(', ', $error['field']));
