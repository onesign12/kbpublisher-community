	...
	class HtmlDiff {
		...
		private function InsertTag( $tag, $cssClass, &$words ) {
			while( true ) {
                ...
                // if( count( $words ) == 0 && count( $specialCaseTagInjection ) == 0 ) { // eleontev
		...
		private function Operations() {
			...
			$matches[] = new Match2( count( $this->oldWords ), count( $this->newWords ), 0 );
		...
		private function FindMatch( $startInOld, $endInOld, $startInNew, $endInNew ) {
			...
			return $bestMatchSize != 0 ? new Match2( $bestMatchInOld, $bestMatchInNew, $bestMatchSize ) : null;
		}
	}
