	...
	{	
		...
		if ($requestMode == XAJAX_POST)
		{
			...
			if (!empty($_POST["xajaxargs"])) 
				...
				// $aArgs = array_map('addslashes', $_POST["xajaxargs"]);
		}
		else
		{	
			...
			if (!empty($_GET["xajaxargs"])) 
				...
				// $aArgs = array_map('addslashes', $_GET["xajaxargs"]);
		}
		...
		if ($bFoundFunction)
		{
			for ($i = 0; $i < sizeof($aArgs); $i++)
			{
				...
				if (0 && is_string($aArgs[$i])) { // get_magic_quotes_gpc deprecated
	...
	function _decodeUTF8Data($sData)
	{
		...
		if ($this->bDecodeUTF8Input)
		{
			...
			else if (strtoupper($this->sEncoding) == "ISO-8859-1")
			{
				...
				$sFuncToUse = "UConverter::transcode";
			}
			else
			{
			...
			if ($sFuncToUse)
			{
				if (is_string($sValue))
				{
					...
					else
					{
                        ...
                        $sValue = UConverter::transcode($sValue, 'ISO-8859-1', 'UTF8', ['to_subst' => '?']);
					}
				}
			}
		}
