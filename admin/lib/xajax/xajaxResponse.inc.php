...
class xajaxResponse
{
	...
	function addRedirect($sURL, $iDelay=0)
	{
        ...
        // $sURL = str_replace($queryPart, $newQueryPart, $sURL); // eleontev March 9, 2021 commented
        $sURL = str_replace('&amp;', '&', $sURL); // eleontev 28 Apr 2018
	...
	function _cmdXML($aAttributes, $sData)
	{
		if ($this->bOutputEntities) {
			if (function_exists('mb_convert_encoding')) {
                ...
                $sData = call_user_func_array('mb_convert_encoding', array(&$sData, 'UTF-8', $this->sEncoding));
                $sData = array_map('htmlentities', $sData);
                $sData = array_map('htmlspecialchars_decode', $sData);
