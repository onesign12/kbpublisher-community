...
class File_Archive_Reader_File extends File_Archive_Reader
{
    ...
    function getData($length = -1)
    {
        ...
        if ($length == -1) {
            ...
            // $saveMagicQuotes = get_magic_quotes_runtime(); // January 19, 2021 deprecated
            // set_magic_quotes_runtime(0); // January 19, 2021 deprecated
            while (!feof($this->handle)) {
                $contents .= fread($this->handle, $blockSize);
            }
            // set_magic_quotes_runtime($saveMagicQuotes); // January 19, 2021 deprecated
            return $contents;
        } else {
            if ($length == 0) {
                return "";
